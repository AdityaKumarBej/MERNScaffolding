...
const buildAPISpec = () => {
    return new Promise((resolve, reject) => {
        logger.info('Building API Specifications');
        
        apiSpec.info                = {}
        apiSpec.info.title          = "API NAME GOES HERE";
        apiSpec.info.version        = packageInfo.version;
        apiSpec.info.description    = "Description of your API";
...
bootExpress()
    ...
    .then(() => {
        app.listen(port, (err) => {
            logger.info(`Server listening on [CONTEXTROOT:${contextRoot}][PORT:${port}]`)
        })
    })
