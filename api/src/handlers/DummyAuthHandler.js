...
router.post('/getAccessToken', (req,res) => {
    try{
        ...
        if(!isPayloadValid.status){
            logger.info(`Missing parameters ${isPayloadValid.missingFields.join(',')}`, res.locals.SVCREQID);
            res.status(400).send(utils.buildMissingFieldResponse(isPayloadValid.missingFields))
        }else{
            let email = req.body.email.trim().toLowerCase();
            let password = req.body.password.trim();

            logger.info(`Access Token Request[${email}]`, res.locals.SVCREQID);

            if(approvedDummyUsers[email] && approvedDummyUsers[email].pwd === password)
            {
                logger.info(`Login Success [${email}]`, res.locals.SVCREQID)
                let accessToken = generateAccessToken({email: email});
                res.status(200).send(accessToken)
            }
            else
            {
                logger.fatal(`Login Failed [${email}]`, res.locals.SVCREQID);
                res.status(401).send({status: false, errMsg: 'Invalid Credentials'});
            }
        }
    }catch(e){
        console.log(`[${TYPE}][${res.locals.SVCREQID}][ERROR]`);
        console.log(e)
...
try{
    ...
    if(!isPayloadValid.status)
    {
        console.log(`[${TYPE}][ERROR][${res.locals.SVCREQID}] Missing Parameters ${isPayloadValid.missingFields.join(',')}`);
        res.status(400).send(utils.buildMissingFieldResponse(isPayloadValid.missingFields))
    }else{
        let email = req.body.email.trim().toLowerCase();
        let password = req.body.password.trim();
...
router.post('/validate', (req,res) => {
    ...
    if(!tokenValidity.result){
        tokenValidity.messages.forEach(message => {
            console.log(`[${TYPE}][ERROR][${res.locals.SVCREQID}] ${message}`);
        });
    }
...
const generateAccessToken = (requestPayload) => {
    ...
    let payload = {
        ...
        'token': createSignature(header.alg, requestPayload.email)
    }
    ...
    */
    ...
    try{
        ...
        if(!verifySignature(header.alg, payload.email, payload.token)){
            //Secondary oken has been tampered with
            tokenIsValid = false;
