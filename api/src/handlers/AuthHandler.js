...
router.post('/getAccessToken', (req,res) => {
    try{
        ...
        if(!isPayloadValid.status){
            console.log(`[${TYPE}][${res.locals.SVCREQID}] Missing parameters ${isPayloadValid.missingFields.join(',')}`);
            ...
            .then((response) => {
                if(response.data && response.data.result){
                    ...
                    console.log(`[${TYPE}][ERROR][${res.locals.SVCREQID}] Login Failed [${email}]`);
